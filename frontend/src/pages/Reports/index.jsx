...
const MOCK_CATEGORIES = [
  { name: 'Assinaturas', amount: 44.90  },
  { name: 'Casa',        amount: 590.00 },
  { name: 'Compras',     amount: 138.90 },
]
    ...
    <div className="rp-root">
      ...
      <div className="rp-shell">
        <header className="rp-header">
          <button className="rp-header__back" onClick={() => navigate('/dashboard')} aria-label="Voltar">
            ...
            {MOCK_CHART_DATA.map((bar) => (
              <div key={bar.day} className="rp-bar-group">
                <div className="rp-bar-track">
                  <div className={`rp-bar${bar.active ? ' rp-bar--active' : ''}`} style={{ height: `${bar.value}%` }} />
                </div>
        ...
        <section className="rp-section">
          ...
          <div className="rp-categories-card">
            {MOCK_CATEGORIES.map((cat, idx) => (
              <button key={cat.name} className="rp-category-row" onClick={() => {}}>
                <div className="rp-category-row__left">
                  <span className="rp-category-row__icon">{getCategoryIcon(cat.name)}</span>
                  <div className="rp-category-row__info">
                    <span className="rp-category-row__name">{cat.name}</span>
                    <span className="rp-category-row__amount">{formatCurrency(cat.amount)}</span>
                  </div>
                </div>
                <span className="rp-category-row__arrow"><IconChevronRight /></span>
                {idx < MOCK_CATEGORIES.length - 1 && <div className="rp-category-divider" />}
              </button>
            ))}
          </div>
        </section>
