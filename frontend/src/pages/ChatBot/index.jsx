...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, type: 'bot', text: "Olá, Juninho! 👋 Sou o Assistente do GNB.\nComo posso te ajudar?" },
  ])
  ...
  const sendMessage = (text) => {
    ...
    setTimeout(() => {
      setIsTyping(false)
      setMessages((prev) => [...prev, { id: Date.now() + 1, type: 'bot', text: getBotResponse(trimmed) }])
    }, 900)

    inputRef.current?.focus()
  }

  const quickQuestions = ['ver saldo', 'extrato', 'devo comprar?']
    ...
    <div className="cb-root">
      ...
      <div className="cb-shell">
        <header className="cb-header">
          <button className="cb-header__back" type="button" onClick={() => navigate('/dashboard')} aria-label="Voltar">
        ...
        <div className="cb-messages" role="log" aria-live="polite">
          {messages.map((msg, idx) => (
            <div key={msg.id} className={`cb-row cb-row--${msg.type}`} style={{ animationDelay: `${idx * 0.03}s` }}>
        ...
        <div className="cb-quick" role="group" aria-label="Perguntas rápidas">
          {quickQuestions.map((q) => (
            <button key={q} className="cb-quick__btn" type="button" onClick={() => sendMessage(q)}>
              {q}
            </button>
          ))}
        </div>
        ...
        <div className="cb-input-area">
          <div className="cb-input-wrap">
            <input
              ...
              aria-label="Mensagem"
            />
            <button className="cb-send" type="button" onClick={() => sendMessage(inputValue)} aria-label="Enviar">
